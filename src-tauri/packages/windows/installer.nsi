...
!macro CheckAllVergeProcesses
  ; Check if bluelayer-mihomo-service.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "bluelayer-mihomo-service.exe"
  !else
    nsis_tauri_utils::FindProcess "bluelayer-mihomo-service.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill bluelayer-mihomo-service.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "bluelayer-mihomo-service.exe"
    !else
      nsis_tauri_utils::KillProcess "bluelayer-mihomo-service.exe"
    !endif
  ${EndIf}

  ; Check if bluelayer-mihomo-alpha.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "bluelayer-mihomo-alpha.exe"
  !else
    nsis_tauri_utils::FindProcess "bluelayer-mihomo-alpha.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill bluelayer-mihomo-alpha.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "bluelayer-mihomo-alpha.exe"
    !else
      nsis_tauri_utils::KillProcess "bluelayer-mihomo-alpha.exe"
    !endif
  ${EndIf}

  ; Check if bluelayer-mihomo.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "bluelayer-mihomo.exe"
  !else
    nsis_tauri_utils::FindProcess "bluelayer-mihomo.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill bluelayer-mihomo.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "bluelayer-mihomo.exe"
    !else
      nsis_tauri_utils::KillProcess "bluelayer-mihomo.exe"
    !endif
  ${EndIf}
...
Section Install
  ...
  ; Always create the desktop shortcut on fresh installs.
  ; CreateOrUpdateDesktopShortcut still respects update/no-shortcut modes.
