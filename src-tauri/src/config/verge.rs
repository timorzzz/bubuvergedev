...
impl IVerge {
    ...
    pub async fn validate_and_fix_config() -> Result<()> {
        ...
        if let Some(ref core) = config.clash_core {
            ...
            if core_str == "verge-mihomo-alpha" {
                logging!(
                    info,
                    Type::Config,
                    "检测到旧的 clash_core 配置 '{}', 将自动迁移为 'bluelayer-mihomo-alpha'",
                    core
                );
                config.clash_core = Some("bluelayer-mihomo-alpha".into());
                needs_fix = true;
            } else if core_str == "verge-mihomo" {
                logging!(
                    info,
                    Type::Config,
                    "检测到旧的 clash_core 配置 '{}', 将自动迁移为 'bluelayer-mihomo'",
                    core
                );
                config.clash_core = Some("bluelayer-mihomo".into());
                needs_fix = true;
            } else if core_str.is_empty() || !Self::VALID_CLASH_CORES.contains(&core_str) {
                logging!(
                    warn,
                    Type::Config,
                    "启动时发现无效的clash_core配置: '{}', 将自动修正为 'bluelayer-mihomo'",
                    core
                );
                config.clash_core = Some("bluelayer-mihomo".into());
                needs_fix = true;
            ...
            logging!(
                info,
                Type::Config,
                "启动时发现未配置clash_core, 将设置为默认值 'bluelayer-mihomo'"
            );
            config.clash_core = Some("bluelayer-mihomo".into());
            needs_fix = true;
        }
    ...
    pub fn template() -> Self {
        Self {
            ...
            clash_core: Some("bluelayer-mihomo".into()),
