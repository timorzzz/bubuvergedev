...
fn uninstall_service() -> Result<()> {
    ...
    let staged = prepare_macos_service_binaries()?;
    let uninstall_path = staged.uninstall_path;

    let uninstall_shell = uninstall_path.to_string_lossy().replace('\'', "'\\''");
...
fn install_service() -> Result<()> {
    ...
    let install_shell = install_path.to_string_lossy().replace('\'', "'\\''");
...
fn ensure_macos_executable(path: &Path) -> Result<()> {
    ...
    if mode & 0o111 != 0o111 {
        ...
        fs::set_permissions(path, permissions)
            .with_context(|| format!("failed to set executable permission for {path:?}"))?;
    }
    Ok(())
}

#[cfg(target_os = "macos")]
struct MacosServiceBinaries {
...
fn prepare_macos_service_binaries() -> Result<MacosServiceBinaries> {
    ...
    let source_uninstall = source_binary.with_file_name(dirs::SERVICE_UNINSTALL_NAME);
