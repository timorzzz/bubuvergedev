...
async fn get_config_values() -> ConfigValues {
    ...
    let (clash_core, enable_tun, enable_builtin, enable_system_proxy, socks_enabled, http_enabled, enable_dns_settings) = (
...
async fn merge_default_config(
    ...
    for (key, value) in clash_config.into_iter() {
        if key.as_str() == Some("tun") {
            ...
            if key.as_str() == Some("port") && !(http_enabled || enable_system_proxy) {
                config.remove("port");
                continue;
            }
