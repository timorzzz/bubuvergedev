...
mod app_init {
    ...
    /// Force the main window to a stable startup size based on the current monitor.
    pub fn setup_main_window_size(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn generate_handlers() -> impl Fn(tauri::ipc::Invoke<tauri::Wry>) -> bool + Send + Sync + 'static {
        tauri::generate_handler![
            ...
            cmd::set_system_proxy_enabled,
            cmd::set_tun_mode_enabled,
            cmd::save_dns_config,
...
pub fn run() {
    ...
    let builder = app_init::setup_plugins(tauri::Builder::default())
        .setup(|app| {
            ...
            if let Err(e) = app_init::setup_main_window_size(app) {
                logging!(error, Type::Setup, "Failed to setup main window size: {}", e);
            }
    ...
    mod event_handlers {
        ...
        pub fn handle_ready_resumed(_app_handle: &AppHandle) {
            ...
            if let Some(window) = _app_handle.get_webview_window("main") {
                let _ = window.set_title("Bluelayer 加速器");
            }
        }
