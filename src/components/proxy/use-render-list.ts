...
export const useRenderList = (
  ...
  const renderList: IRenderItem[] = useMemo(() => {
    ...
    const allRenderGroups =
      useRule && proxiesData.groups.length
        ? proxiesData.groups
        : [proxiesData.global!]

    const preferredNames = ['🔰国外流量', '国外流量']
    const preferredMatches = allRenderGroups.filter((group: ProxyGroup) =>
      preferredNames.some((name) => group.name === name) ||
      preferredNames.some((name) => group.name.includes(name)),
    )

    const selectorFallback = allRenderGroups.filter(
      (group: ProxyGroup) => group.type === 'Selector' && !group.hidden,
    )

    const renderGroups = useRule
      ? preferredMatches.length
        ? preferredMatches.slice(0, 1)
        : selectorFallback.length
          ? selectorFallback.slice(0, 1)
          : allRenderGroups.slice(0, 1)
      : allRenderGroups
